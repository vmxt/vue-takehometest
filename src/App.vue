  ...
  <div class="p-6 w-3xl text-white rounded-lg font-sans">
    ...
    <div class="flex gap-3 mb-6">
      ...
      <button @click="addTopic" 
        class="bg-dark-600 text-white px-5 py-3 rounded-lg hover:bg-dark-700 transition">Add</button>
    </div>
    
    <ul class="space-y-4">
      <li v-for="topic in paginatedTopics" :key="topic.guid" class="p-5 bg-dark-800 rounded-lg shadow-md">
        <div class="flex justify-between items-center">
          <span v-if="!topic.editing" @click="selectTopic(topic)" class="cursor-pointer font-semibold text-light hover:underline w-full overflow-hidden truncate">
            ID:{{ topic.guid }} - <span class="truncate w-60inline-block align-middle">{{ topic.name }}</span>
          </span>
        ...
        <div v-if="selectedTopic && selectedTopic.guid === topic.guid" class="mt-4 border-t border-gray-700 pt-3">
          <h2 class="text-lg font-semibold">Comments:</h2>
          <ul v-if="topic.comments.length" class="mt-2 space-y-2">
            <li v-for="comment in topic.comments" :key="comment.date" 
              class="p-3 bg-dark-700 rounded-md">
  ...
  setup() {
    ...
    const fetchTopics = async () => {
      try {
        ...
        topics.value = data.topics.map(topic => ({ ...topic, editing: false }));
      } catch (error) {
    ...
    const deleteTopic = (guid) => {
      ...
      if (selectedTopic.value && selectedTopic.value.guid === guid) {
    ...
    const nextPage = () => {
      if (currentPage.value < Math.ceil(topics.value.length / itemsPerPage)) {
        currentPage.value++;
      }
    };
